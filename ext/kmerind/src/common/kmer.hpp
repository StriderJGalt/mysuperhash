...
//      std::cout << std::endl;
    ...
    //constructing from begin and end iterators of a vector of char type.
    using CharVectorConstIterator = typename std::vector<char>::const_iterator;
    explicit Kmer(CharVectorConstIterator begin, CharVectorConstIterator end)
    {
      // assert if difference between begin and end is equal to size of kmer
      assert(std::distance(begin, end) == Kmer::size);

      // copy data from the iterator using fill from char
      for (auto it = begin; it != end; ++it) {
        this->nextFromChar(*it);
      }
    }
