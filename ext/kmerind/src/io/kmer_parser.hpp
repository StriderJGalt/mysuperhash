...
// The minimizer is the kmer that is lexicographically smallest in the supermer.
/**
 * @tparam KmerType       output value type of this parser.  not necessarily the same as the map's final storage type.
 * @tparam MinimizerType contain minimizer size and has comparison operator
 */
template <typename KmerType, typename MinimizerType>
class SupermerTupleParser {

  public:
    using kmer_type = KmerType;
    using minimizer_type = MinimizerType;
    // using value_type = ::std::tuple<minimizer_type, vector<typename KmerType::KmerAlphabet>>;
    // using value_type = ::std::tuple<minimizer_type, array<char, 100>>; // testing using array instead of vector since mxx does not support vector
    using value_type = ::std::tuple<minimizer_type, vector<char>>; //testing using vector of char
    static constexpr size_t window_size = kmer_type::size;
    static constexpr size_t minimizer_size = minimizer_type::size;
  ...
  // mmers generation iterator
  template <typename SeqType>
  using mmer_iterator_type = bliss::common::KmerGenerationIterator<BaseCharIterator<SeqType>, minimizer_type>;
  ...
  /**
   * @brief generate supermer tuples from 1 sequence.  result inserted into output_iter, which may be preallocated.
   * @param read          sequence object, which has pointers to the raw byte array.
   * @param output_iter   output iterator pointing to insertion point for underlying container.
   * @return new position for output_iter
   * @tparam SeqType      type of sequence.  inferred.
