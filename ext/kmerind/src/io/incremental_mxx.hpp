...
//        std::cout << "i2o read " << counter << " times.  size= " << i2o.size() << std::endl;
    ...
    // function to concatenate all supermers in input into a single supermer 
    // add length of each supermer in supermer_lengths
    // update send_counts to have number of elements of supermer type to be sent to each rank
    // update send_counts_for_supermer_lengths to have number of elements to be sent to each rank
    template <typename INPUT_TUPLE_TYPE, typename SIZE_TYPE>
    void serialize_supermers(const std::vector<INPUT_TUPLE_TYPE> & input, 
    ...
    //extract individual supermers from concatenated supermers using supremer_lengths
    
    template <typename SUPERMER_TYPE, typename SIZE_TYPE>
    void unserialize_supermers(const SUPERMER_TYPE & concatenated_supermers,
      std::vector<SIZE_TYPE> & supermer_lengths, 
      std::vector<SUPERMER_TYPE> & output) {
        ...
        SIZE_TYPE supermer_start_index = 0;
        for(auto supermer_length : supermer_lengths) {
          output.push_back(SUPERMER_TYPE(concatenated_supermers.begin() + supermer_start_index, concatenated_supermers.begin() + supermer_start_index + supermer_length));
          supermer_start_index += supermer_length;
        }
...
//    assert((send_count * comm.size() < mxx::max_int) && "send count too large for mpi ");
  ...
  void distribute_supermers(::std::vector<V>& input, ToRank const & to_rank,
    ...
    // // print input on process 0 for testing 
    // if (_comm.rank() == 0) {
    ...
    BL_BENCH_COLLECTIVE_START(distribute, "empty", _comm);
    bool empty = input.size() == 0;
    empty = mxx::all_of(empty); // empty is true if input is empty in all process and false otherwise
    BL_BENCH_END(distribute, "empty", input.size());
    ...
    // bucketing
    BL_BENCH_START(distribute);
    imxx::local::assign_to_buckets(input, to_rank, _comm.size(), send_counts, i2o, 0, input.size()); //i2o will have id of bucket corresponding to each element of input and send_counts will have number of elements in each bucket will have no: of elements in each bucket
    BL_BENCH_COLLECTIVE_END(distribute, "bucket", input.size(), _comm);

    BL_BENCH_START(distribute);
    // distribute (communication part)
    imxx::local::bucket_to_permutation(send_counts, i2o, 0, input.size()); //i2o will have position where each element of input should be placed in the output
    BL_BENCH_COLLECTIVE_END(distribute, "to_pos", input.size(), _comm);

    BL_BENCH_START(distribute);
    vector<V> output;
    // if (output.capacity() < input.size()) output.clear(); // no longer necessary?
    output.resize(input.size());
    output.swap(input);  // swap the 2.
    BL_BENCH_COLLECTIVE_END(distribute, "alloc_permute", output.size(), _comm);
    ...
    //     std::cout << "initial send_counts: ";
    //     for (auto const & i : send_counts) {
    //       std::cout << i << " ";
    //     }
    //     std::cout << std::endl;
    //     std::cout << "input size: " << input.size() << std::endl;
    //     for (auto const & t : input) {
    //       auto mmer = ::std::get<0>(t);
    //       supermer_type supermer = ::std::get<1>(t);
    //       std::cout << mmer << " ";
    //       for (char c : supermer) {
    //         if(c==0)
    //           std::cout << "A";
    ...
    //     for(auto x : supermer_lengths_to_be_sent) std::cout << x << " ";
    //     std::cout << endl;
    //     std::cout << "send_counts: ";
    //     for(auto x : send_counts) std::cout << x << " ";
    //     std::cout << endl;
    //     std::cout << "send_counts_for_supermer_lengths: ";
    //     for(auto x : send_counts_for_supermer_lengths) std::cout << x << " ";
    //     std::cout << endl;
    //   }
    //   _comm.barrier();
    // }
    // return;
    // vector<supermer_type> packed_supermers;
    // vector<SIZE> recv_counts_for_testing(_comm.size(), 0);
    // recv_counts_for_testing[_comm.rank()] = input.size(); 
    // imxx::local::unserialize_supermers<supermer_type, SIZE>(concatenated_supermers_to_be_sent, supermer_lengths_to_be_sent, packed_supermers);
    // for(int i = 0; i < _comm.size(); i++) {
    //   if(i == _comm.rank()) {
    //     std::cout << "rank: " << i << endl;
    //     std::cout << "packed_supermers size: " << packed_supermers.size() << endl;
    //     for(int i=0; i< packed_supermers.size(); i++) {
    //       if(packed_supermers[i] != std::get<1>(input[i])) {
    //         std::cout << "packed_supermers[i] != std::get<1>(input[i])" << std::endl;
    //         std::cout << "error in packing" << std::endl;
    //       }
    //       else {
    //         std::cout << "packed_supermers[i] == std::get<1>(input[i])" << std::endl;
    //       }
    //         std::cout << "packed_supermers[" << i << "]: ";
    //         for(auto c : packed_supermers[i]) {
    //           if(c == 0) 
    //             std::cout << "A";
    //           else if(c == 1)
    //             std::cout << "C";
    ...
    //         }
    //         std::cout << endl;
    //         std::cout << "std::get<1>(input[" << i << "]): ";
    //         for(auto c : std::get<1>(input[i])) {
    //           if(c == 0) 
    //             std::cout << "A";
    //           else if(c == 1)
    //             std::cout << "C";
    ...
    recv_counts.resize(_comm.size());
    mxx::all2all(send_counts.data(), 1, recv_counts.data(), _comm);
    size_t total = std::accumulate(recv_counts.begin(), recv_counts.end(), static_cast<size_t>(0)); // total will contain total no: of characters in the concatenated_supermers
    BL_BENCH_COLLECTIVE_END(distribute, "a2a_count", recv_counts.size(), _comm);

    BL_BENCH_START(distribute);
    // now resize output
    if (concatenated_supermers.capacity() < total) concatenated_supermers.clear();
    concatenated_supermers.resize(total);
    BL_BENCH_COLLECTIVE_END(distribute, "realloc_out", concatenated_supermers.size(), _comm);

    BL_BENCH_START(distribute);
    mxx::all2allv(concatenated_supermers_to_be_sent.data(), send_counts, concatenated_supermers.data(), recv_counts, _comm);
    BL_BENCH_END(distribute, "a2a", concatenated_supermers.size());

    // communicate send counts for supermer_lengths
    recv_counts.clear(); // this and immediately below line can be omited as recv_counts can simply be overwritten in the next all2all
    recv_counts.resize(_comm.size());
    mxx::all2all(send_counts_for_supermer_lengths.data(), 1, recv_counts.data(), _comm);
    
    size_t total_supermer_lengths = std::accumulate(recv_counts.begin(), recv_counts.end(), static_cast<size_t>(0)); // total will contain total no: of elements in supermer_lengths
    
    // allocate buffer to receive supermer_lengths
    if (supermer_lengths.capacity() < total_supermer_lengths) supermer_lengths.clear();
    supermer_lengths.resize(total_supermer_lengths);

    // communicate supermer_lengths
    mxx::all2allv(supermer_lengths_to_be_sent.data(), send_counts_for_supermer_lengths, supermer_lengths.data(), recv_counts, _comm);

    // // printing for debugging----------------------------------------------
    // // print the vectors concatenated_supermers, supermer_lengths to check if they are correct
    // // use mpi barrier to make sure that one process has printed all the vectors before proceeding to next process
    // for(int i = 0; i < _comm.size(); i++) {
    //   if(i == _comm.rank()) {
    //     std::cout << "rank: " << i << endl;
    //     std::cout << "concatenated_supermers: ";
    //     for(auto c : concatenated_supermers) {
    //       if(c == 0) 
    //         std::cout << "A";
    //       else if(c == 1)
    //         std::cout << "C";
    ...
    //     } 
    //     std::cout << endl;
    //     std::cout << "supermer_lengths: ";
    //     for(auto x : supermer_lengths) std::cout << x << " ";
    //     std::cout << endl;
    //     std::cout << "recv_counts: ";
    //     for(auto s : recv_counts) {
    //       std::cout << s << " ";
    //     }
    //     std::cout << endl;
    //   }
    ...
    //     for(auto x : supermer_lengths) std::cout << x << " ";
    //     std::cout << endl;
    //     std::cout << "supermers: " << endl;
    //     for(auto s : supermers) {
    //       for(auto c : s) {
    //         if(c == 0) 
    //           std::cout << "A";
    //         else if(c == 1)
